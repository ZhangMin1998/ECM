...
export default {
  data() {
    return {
      ...
      //图片上传组件的headers请求对象
      headerObj: {
        Authorization: window.sessionStorage.getItem("token") ///////////////////////////////难点
      },
      //预览图片的路径
      previewPath: "",
      //是否显示预览对话框
  ...
  computed: {
    cateId() {
      if (this.addForm.goods_cat.length === 3) {
        return this.addForm.goods_cat[2]; //2 就是三级
      }
      return null;
    }
  },
  methods: {
    ...
    add() {
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        form.goods_cat = form.goods_cat.join(","); //把goods_cat从数组转化成字符串
        //console.log(form);

        // 处理动态参数
        this.manyTableData.forEach(item => {
          const newInfo = {
            attr_id: item.addr_id,
